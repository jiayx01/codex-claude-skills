...
```mermaid
flowchart TD
    A["备料：背景、约束、判断标准、相关文件绝对路径"] --> B["独立起草：Claude 和 Codex 各写一版，互不参考"]
    B --> C["分类差异：共识点 / 单边点 / 冲突点"]
    C --> D["只追问单边点和冲突点"]
    D --> E{"还有新分歧，且未满 3 轮？"}
    E -->|是| C
    E -->|否| F["核实事实前提：能查的自己查，不转述模型的话"]
    F --> G["自己决断，交付一版方案"]
```
...
```mermaid
flowchart TD
    A["Claude 规划拆解任务"] --> B["写任务书委派给 Codex：绝对路径 + 已尝试方案 + 失败原因"]
    B --> C["Codex 执行：改代码 / 跑测试 / 改 bug"]
    C --> D["Claude 验收：自己重跑测试 + 读 diff 核对范围"]
    D --> E{"通过？"}
